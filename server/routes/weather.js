...
router.get('/background', asyncHandler(async (req, res) => {
    if (process.env.USE_MOCK_DATA === 'true') {
        ...
        return res.json({
            ...
            user: { name: "Mode Test", username: "test_user" },
    ...
    if (!process.env.UNSPLASH_KEY) {
        throw new AppError("Clé API Unsplash manquante (côté serveur)", 500);
    }
    ...
    try {
        ...
        // Si Unsplash échoue, on renvoie une erreur propre
        throw new AppError("Erreur lors de la récupération de l'image Unsplash", 502);
    }
}));
